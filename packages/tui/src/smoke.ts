...
assert(first.includes('[d] 详情'), '底部快捷键栏应包含 [d] 详情');
assert(first.includes('[s] 统计'), '底部快捷键栏应包含 [s] 统计');
...
const rev = frame();
assert(rev.includes('outcome 复盘'), '[o] 弹层标题应为「outcome 复盘」');
process.stderr.write('smoke: [o] outcome 复盘弹层 OK\n');
setup.mockInput.pressEscape();
await sleep(200);
