...
- 从环境变量读取 `ADSHARE_URL` + `ADSHARE_API_KEY`。
...
| 错误码 | 场景 |
|--------|------|
| `config_missing` | `ADSHARE_URL` 未设置 |
| `network_error` | 请求远端失败 |
| `http_error` | 远端返回非 2xx |
| `parse_error` | 响应解析 / Zod 校验失败 |
| `not_found` | 无 K 线或行情数据 |
| `invalid_input` | 搜索参数缺少 `name` 和 `ts_code` |

## 默认值来源

| 参数 | 默认值 | 出处 |
|------|--------|------|
| `timeoutMs` | `10_000` ms（10 s） | `client.ts`：`options.timeoutMs ?? 10_000` |
...
调用方可通过 `new AdshareClient({ timeoutMs, retries, ... })` 覆盖；`apps/web` 集成中未显式覆盖，沿用上述默认值。
...
SDK 仅暴露两个运行期参数，且对应一次完整的请求生命周期：

| 参数 | 默认值 | 取值依据 |
|------|--------|----------|
| `timeoutMs` | `10_000` ms（10 s） | 整体超时上限（连接 + 读 body）。生产到 adshare 服务的 RTT 通常 < 50 ms（内网）或 < 200 ms（同城专线），10 s 留出 50–200 倍 headroom 容纳 GC 短暂停顿、上游冷启动、慢查询。低于 5 s 在弱网下误杀率过高；高于 30 s 又会让同步接口在用户侧卡死。 |
...
- **搜索接口**（`/stock_basic`）：保持默认。前端有本地 mock 兜底，单次超时 → 走本地，不影响功能。
