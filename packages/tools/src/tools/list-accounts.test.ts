...
describe('list_accounts', () => {
  it('正常路径：返回全部账户', async () => {
    ...
    expect(ids).toContain(TEST_ACCOUNT.id);
    expect(ids).toContain(TEST_ACCOUNT_LONGTERM.id);
    expect(ids).toContain(TEST_ACCOUNT_SHORTTERM.id);
    const names = result.data.accounts.map((a) => a.name);
    expect(names).toContain(TEST_ACCOUNT.name);
    expect(names).toContain(TEST_ACCOUNT_LONGTERM.name);
    expect(names).toContain(TEST_ACCOUNT_SHORTTERM.name);
  });
