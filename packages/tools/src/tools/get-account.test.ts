...
describe('get_account', () => {
  it('正常路径：缺省 accountId → 默认账户', async () => {
    ...
    if (!result.ok) return;
    expect(result.data.account.id).toBe(TEST_ACCOUNT.id);
  ...
  it('正常路径：显式 accountId', async () => {
    const ctx = await buildTestContext();
    const result = await getAccountTool.execute({ accountId: TEST_ACCOUNT.id }, ctx);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.data.account.name).toBe(TEST_ACCOUNT.name);
  });
