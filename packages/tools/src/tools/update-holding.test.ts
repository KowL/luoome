...
describe('update_holding', () => {
  it('只改传入字段：avgCost', async () => {
    ...
    const result = await updateHoldingTool.execute(
      { holdingId: 'mock-holding-002594', avgCost: 95.5 },
      ctx,
    );
  ...
  it('同时改 quantity + availableQuantity', async () => {
    const ctx = await buildMockContext();
    const result = await updateHoldingTool.execute(
      { holdingId: 'mock-holding-002594', quantity: 800, availableQuantity: 700 },
      ctx,
    );
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it('空更新（无任何字段）→ invalid_input', async () => {
    const ctx = await buildMockContext();
    const result = await updateHoldingTool.execute({ holdingId: 'mock-holding-002594' }, ctx);
  ...
  it('合并后 availableQuantity > quantity → invalid_input', async () => {
    const ctx = await buildMockContext();
    const result = await updateHoldingTool.execute(
      { holdingId: 'mock-holding-002594', quantity: 100 },
      ctx,
    );
