...
describe('close_holding', () => {
  it('平仓：closedAt 落当前时钟，行保留', async () => {
    ...
    const result = await closeHoldingTool.execute({ holdingId: 'mock-holding-002594' }, ctx);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.data.holding.closedAt).not.toBeNull();

    const persisted = await ctx.repos.holding.findById('mock-holding-002594');
  ...
  it('重复平仓 → invalid_input', async () => {
    const ctx = await buildMockContext();
    const first = await closeHoldingTool.execute({ holdingId: 'mock-holding-002594' }, ctx);
    expect(first.ok).toBe(true);
    const second = await closeHoldingTool.execute({ holdingId: 'mock-holding-002594' }, ctx);
