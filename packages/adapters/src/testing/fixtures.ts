...
export const TEST_STOCKS: readonly Stock[] = [
  // A 股 ×10
...
/** 各 fixture 股票的 测试基准价（close），供行情 adapter 与持仓成本对齐。 */
export const TEST_STOCK_BASE_PRICES: Readonly<Record<string, Money>> = {
...
export const findTestStock = (stockIdOrCode: string): Stock | null => {
...
export const TEST_ACCOUNT_LONGTERM: Account = {
...
export const TEST_ACCOUNT_SHORTTERM: Account = {
...
/** 全部测试账户（默认账户 + 长期 + 短线），供 list_accounts / seed 使用。 */
export const TEST_ACCOUNTS: readonly Account[] = [
  TEST_ACCOUNT,
  TEST_ACCOUNT_LONGTERM,
  TEST_ACCOUNT_SHORTTERM,
];

// ---------- 持仓：6 个（引用 TEST_STOCKS / TEST_ACCOUNT） ----------
...
export const TEST_HOLDINGS: readonly Holding[] = [
  holding('test-holding-002594', '002594.SZ', 1000, 98.5, '2026-05-06T02:30:00.000Z'),
  holding('test-holding-600519', '600519.SH', 100, 1450.0, '2026-04-15T02:30:00.000Z'),
  holding('test-holding-00700', '00700.HK', 500, 480.0, '2026-03-20T02:30:00.000Z'),
  holding('test-holding-aapl', 'AAPL.US', 50, 195.0, '2026-02-10T15:30:00.000Z'),
  holding('test-holding-300750', '300750.SZ', 400, 250.0, '2026-05-28T02:30:00.000Z'),
  holding('test-holding-600036', '600036.SH', 2000, 39.8, '2026-06-03T02:30:00.000Z'),
];
...
export const TEST_TRADES: readonly Trade[] = [
  // 002594.SZ：600 @96.00 + 400 @102.25 → 加权 98.50
  trade('test-trade-0001', '002594.SZ', 600, 96.0, 14.4, '2026-05-06T02:30:00.000Z'),
  trade('test-trade-0002', '002594.SZ', 400, 102.25, 10.23, '2026-05-06T06:30:00.000Z'),
  // 600519.SH：100 @1450.00
  trade('test-trade-0003', '600519.SH', 100, 1450.0, 36.25, '2026-04-15T02:30:00.000Z'),
  // 00700.HK：300 @460.00 + 200 @510.00 → 加权 480.00
  trade('test-trade-0004', '00700.HK', 300, 460.0, 27.6, '2026-03-20T02:30:00.000Z'),
  trade('test-trade-0005', '00700.HK', 200, 510.0, 20.4, '2026-03-21T02:30:00.000Z'),
  // AAPL.US：50 @195.00
  trade('test-trade-0006', 'AAPL.US', 50, 195.0, 1.95, '2026-02-10T15:30:00.000Z'),
  // 300750.SZ：200 @245.00 + 200 @255.00 → 加权 250.00
  trade('test-trade-0007', '300750.SZ', 200, 245.0, 12.25, '2026-05-28T02:30:00.000Z'),
  trade('test-trade-0008', '300750.SZ', 200, 255.0, 12.75, '2026-05-29T02:30:00.000Z'),
  // 600036.SH：2000 @39.80
  trade('test-trade-0009', '600036.SH', 2000, 39.8, 19.9, '2026-06-03T02:30:00.000Z'),
];
 ...
 */
export const testAdviceFor = (
  stockId: string,
  clock: () => Date = () => new Date(DEFAULT_TEST_NOW.getTime()),
): Advice => {
  const now = clock();
  ...
  const known = findTestStock(stockId);
  const resolvedStockId = known ? known.id : stockId;

  return {
    id: `test-advice-${seed.toString(16).padStart(8, '0')}`,
    subjectKind: 'stock',
    ...
    horizon,
    reasoning: {
      premise: `测试分析：${resolvedStockId} 当前信号指向「${decision}」（确定性 fixture）。`,
      evidence: [
        `测试证据：${resolvedStockId} 基准价与 60 日线偏离处于历史中枢`,
        '测试证据：近 20 日量能温和，未出现异常放量',
      ],
      counterEvidence: ['测试反证：板块轮动与大盘系统性风险未纳入 测试模型'],
    },
    risks: ['测试风险：本 fixture 不反映真实基本面', '测试风险：大盘系统性下行风险'],
