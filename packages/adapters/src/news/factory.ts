...
import { TenjqkaNewsSource } from './10jqka.js';
 ...
 * - LUOOME_NEWS_SOURCES：逗号分隔、有序、去重；未知源启动期抛错；缺省 eastmoney,10jqka
...
export interface CreateNewsManagerDeps {
  ...
  readonly sources?: { readonly eastmoney?: EastmoneySource; readonly tenjqka?: TenjqkaNewsSource };
}

/** 已注册的财经要闻数据源（封闭启动校验；core 端口侧是开放 SourceIdSchema）。 */
const NewsSourcesSchema = z
  .array(z.enum(['eastmoney', '10jqka']))
  .min(1, '至少启用一个财经要闻数据源')
...
export const createNewsManagerFromEnv = (
  ...
  const tenjqka = deps.sources?.tenjqka ?? new TenjqkaNewsSource(deps.fetchImpl);
  ...
  const bindings = order.flatMap((source) => {
    switch (source) {
      ...
      case '10jqka':
        return tenjqkaBindings(tenjqka);
      default:
 ...
 */
const observationOfNews = (result: NewsCapabilityMap['finance-news']['result']) => {
  ...
  return latest === undefined
    ...
    : ({ outcome: 'success', dataAsOf: latest } as const);
};
...
const eastmoneyBindings = (source: EastmoneySource): AnyBinding<NewsCapabilityMap>[] => [
  {
    ...
    execute: ({ page, pageSize }) => source.fetchNews(page, pageSize),
    observationOf: observationOfNews,
  },
];

const tenjqkaBindings = (source: TenjqkaNewsSource): AnyBinding<NewsCapabilityMap>[] => [
  {
    capability: 'finance-news',
    source: '10jqka',
    coverage: ['CN_FINANCE_NEWS'],
