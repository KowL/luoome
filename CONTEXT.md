...
回答“这个账户现在有多少现金、多少持仓市值、总资产是多少”。每次人工更新产生新的不可变版本，
`(accountId, version)` 唯一；`complete` 版本必须满足 `totalAssets = cashBalance + stockMarketValue`，
且 `stockMarketValue` 等于持仓市值之和。只更新持仓而未同步资金时标记 `needs-reconciliation`：
保留现金与持仓事实，但不得暴露精确 `totalAssets`，下游也不能据此给出精确仓位或当前可执行的
增仓建议。快照是用户维护的账户事实，建议与行情变化都不改写它。
