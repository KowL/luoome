 ...
 * （system 首行的 'chat_plan' 是 MockLLMAdapter 的分派标识，勿删。）
 */
...
/**
 * Pass 2 system prompt（'chat_reply' 是 MockLLMAdapter 分派标识，勿删）。
 */
...
export const handleChat = async (
  ...
  try {
    ...
    // adapter 契约是「先 parse 再返回」，但 mock / 自定义 adapter 可能返回未校验输出，兜底再 parse
