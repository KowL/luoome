...
- LLM 必须由 `LUOOME_LLM_PROVIDER` 配置真实模型；调用失败时返回明确的保守 fallback
...
- LLM 失败时回复兜底文案，UI 不特殊处理

### 6. 测试

- `server.test.ts` 补 `/api/chat` 用例：测试 fake LLM 返回 fallback reply；构造 plan 含 write action → 只进 drafts 不执行；action 超白名单被拦截；history 截断生效
- draft 校验失败丢弃路径覆盖
