...
> 变更（2026-09-20，用户决定）：取消「同一行业股票仓位合计上限」。本地股票目录没有行业分类数据
> （`stocks.industry` 全为空），该校验无法执行，而实现把「无法校验」当成「不可用」，导致所有带
> 增仓量的计划永久停在草案。组合预算只保留总仓位上限与单只股票上限；行业分类仍作为基础资料展示，
> 不参与预算判定。
> 变更（2026-09-20，用户决定）：总仓位上限默认值由 80% 调整为 100%。原 80% 属"建议上限"，
> 账户一旦用满即让所有增量计划永久停在草案；调整后只保留「建议仓位合计不得超过账户总资产」
> 这条硬底线（不新增融资路径）。
> 变更（2026-09-20，用户决定）：单只股票上限默认值由 15% 调整为 30%。集中度由用户自行判断，
> 已有超过 30% 的持仓仍可减仓，但不能继续加仓。
...
| 规则 | 本版默认值 |
|---|---|
| 建议股票总仓位上限 | 100%（仅作"不得超过账户总资产"的硬底线） |
| 建议单只股票仓位上限 | 30% |
...
- 组合预算只校验总仓位与单只股票上限；行业分类不参与判定，因此也不宣称行业集中度已校验
  （本地目录缺行业数据，且判断缺失不等于违规）。
