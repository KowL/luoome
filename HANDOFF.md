...
| 测试 | 418 pass（vitest 361 + bun test db 57）/ 50 文件 |
...
| 数据源 | Eastmoney（A 股 / 港股） + Tencent（A 股备用） + Mock；OpenAI-Compatible / Anthropic / Mock LLM；飞书 Webhook |
...
1. **多市场数据源**（港股 Tencent 已接 / 美股 Yahoo / Alpha Vantage）—— `Stock.market` 字段已就位
2. **多账户切换 UI**（侧栏加账户选择器）
